...
function renderComposerFiles(files) {
  ...
  for (const file of files) {
    ...
    statAdd.className = "diff-stat-add";
    statAdd.textContent = `+${file.additions}`;
    const statDel = document.createElement("span");
    statDel.className = "diff-stat-del";
    statDel.textContent = `−${file.deletions}`;
    stat.append(statAdd, " ", statDel);
...
function renderComposerProposal(data) {
  ...
  composerStats.replaceChildren();
  composerStats.append(`${files.length} arquivo(s) · `);
  const statsAdd = document.createElement("span");
  statsAdd.className = "diff-stat-add";
  statsAdd.textContent = `+${add}`;
  const statsDel = document.createElement("span");
  statsDel.className = "diff-stat-del";
  statsDel.textContent = `−${del}`;
  composerStats.append(statsAdd, " ", statsDel);
