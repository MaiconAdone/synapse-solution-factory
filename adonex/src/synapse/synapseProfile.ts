...
export function synapseSystemContext(
  ...
  const childProjectNote = projectIdentity && !projectIdentity.isPlatformRepo
    ? `This workspace is the project "${projectIdentity.name}"${
        projectIdentity.universe ? ` (universe: ${projectIdentity.universe})` : ""
      } — it is NOT the Synapse platform repository itself${
        projectIdentity.universe ? " (it was generated by the Synapse Solution Factory)" : ""
      }. Apply Synapse's engineering conventions here when relevant, but always refer to this workspace by its real name "${projectIdentity.name}", never as "Synapse".`
    : "";
  return [
